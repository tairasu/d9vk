...
public:
  DxvkAdapter(const Rc<vk::InstanceFn> &vki, VkPhysicalDevice handle);
  ~DxvkAdapter();
   ...
   */
  const DxvkDeviceInfo &devicePropertiesExt() const { return m_deviceInfo; }
   ...
   */
  const DxvkDeviceFeatures &features() const { return m_deviceFeatures; }
   ...
   */
  VkResult imageFormatProperties(VkFormat format, VkImageType type,
   ...
   */
  bool checkFeatureSupport(const DxvkDeviceFeatures &required) const;
   ...
   */
  void enableExtensions(const DxvkNameSet &extensions);
   ...
   */
  Rc<DxvkDevice> createDevice(const Rc<DxvkInstance> &instance,
   ...
   */
  void notifyHeapMemoryAlloc(uint32_t heap, VkDeviceSize bytes);
   ...
   * \param [in] bytes Allocation size
   */
  void notifyHeapMemoryFree(uint32_t heap, VkDeviceSize bytes);
   ...
   */
  bool matchesDriver(DxvkGpuVendor vendor, VkDriverIdKHR driver,
  ...
  /**
   * \brief Checks whether the driver is MoltenVK
   * \returns \c true if the driver is MoltenVK
   */
  bool isMoltenVK() const { return m_isMoltenVK; }
