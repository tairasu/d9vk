...
DxvkMemory::DxvkMemory(DxvkMemoryAllocator *alloc, DxvkMemoryChunk *chunk,
                       DxvkMemoryType *type, VkDeviceMemory memory,
                       VkDeviceSize offset, VkDeviceSize length, void *mapPtr)
    : m_alloc(alloc), m_chunk(chunk), m_type(type), m_memory(memory),
      m_offset(offset), m_length(length), m_mapPtr(mapPtr) {}
...
DxvkMemory::~DxvkMemory() { this->free(); }
...
DxvkMemoryChunk::DxvkMemoryChunk(DxvkMemoryAllocator *alloc,
                                 DxvkMemoryType *type, DxvkDeviceMemory memory,
...
DxvkMemory DxvkMemoryChunk::alloc(VkMemoryPropertyFlags flags,
  ...
  return DxvkMemory(m_alloc, this, m_type, m_memory.memHandle, allocStart,
                    allocEnd - allocStart,
                    reinterpret_cast<char *>(m_memory.memPointer) + allocStart);
}

void DxvkMemoryChunk::free(VkDeviceSize offset, VkDeviceSize length) {
...
bool DxvkMemoryChunk::isCompatible(const Rc<DxvkMemoryChunk> &other) const {
  return other->m_memory.memFlags == m_memory.memFlags &&
         other->m_hints == m_hints;
}

bool DxvkMemoryChunk::checkHints(DxvkMemoryFlags hints) const {
  DxvkMemoryFlags mask(DxvkMemoryFlag::Small, DxvkMemoryFlag::GpuReadable,
                       DxvkMemoryFlag::GpuWritable, DxvkMemoryFlag::Transient);
...
DxvkMemoryAllocator::DxvkMemoryAllocator(const DxvkDevice *device)
  ...
  for (uint32_t i = 0; i < m_memProps.memoryTypeCount; i++) {
    if (m_memTypes[i].memType.propertyFlags &
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
      largestDeviceLocalHeap =
          std::max(largestDeviceLocalHeap, m_memTypes[i].heap->properties.size);
  }
  ...
  if (m_device->properties().core.properties.vendorID ==
      uint16_t(DxvkGpuVendor::Nvidia)) {
    bool shrinkNvidiaHvvHeap = device->adapter()->matchesDriver(
        DxvkGpuVendor::Nvidia, VK_DRIVER_ID_NVIDIA_PROPRIETARY_KHR, 0,
    ...
    if (shrinkNvidiaHvvHeap) {
      for (uint32_t i = 0; i < m_memProps.memoryTypeCount; i++) {
        VkMemoryPropertyFlags hvvFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
...
DxvkMemory
DxvkMemoryAllocator::alloc(const VkMemoryRequirements *req,
                           const VkMemoryDedicatedRequirements &dedAllocReq,
                           const VkMemoryDedicatedAllocateInfo &dedAllocInfo,
                           VkMemoryPropertyFlags flags, DxvkMemoryFlags hints) {
  ...
  auto dedAllocPtr =
      dedAllocReq.prefersDedicatedAllocation ? &dedAllocInfo : nullptr;
  ...
  if (!result) {
    DxvkAdapterMemoryInfo memHeapInfo =
    ...
    Logger::err(str::format(
        ...
        std::hex, req->memoryTypeBits));

    for (uint32_t i = 0; i < m_memProps.memoryHeapCount; i++) {
      Logger::err(str::format(
          "Heap ", i, ": ", (m_memHeaps[i].stats.memoryAllocated >> 20),
          " MB allocated, ", (m_memHeaps[i].stats.memoryUsed >> 20),
          " MB used, ",
          m_device->extensions().extMemoryBudget
              ? str::format((memHeapInfo.heaps[i].memoryAllocated >> 20),
                            " MB allocated (driver), ",
                            (memHeapInfo.heaps[i].memoryBudget >> 20),
                            " MB budget (driver), ",
                            (m_memHeaps[i].properties.size >> 20), " MB total")
              : str::format((m_memHeaps[i].properties.size >> 20),
                            " MB total")));
    }
...
DxvkMemory
DxvkMemoryAllocator::tryAlloc(const VkMemoryRequirements *req,
                              const VkMemoryDedicatedAllocateInfo *dedAllocInfo,
                              VkMemoryPropertyFlags flags,
                              DxvkMemoryFlags hints) {
  DxvkMemory result;

  bool isMoltenVK = m_device->adapter()->isMoltenVK();

  for (uint32_t i = 0; i < m_memProps.memoryTypeCount && !result; i++) {
    const bool supported = (req->memoryTypeBits & (1u << i)) != 0;
    const bool adequate =
        (m_memTypes[i].memType.propertyFlags & flags) == flags;

    if (supported && adequate) {
      // MoltenVK optimization: Prefer Private memory (Device Local only) over
      // Shared/Managed (Device Local + Host Visible) when Host Visible is not
      // requested.
      if (isMoltenVK && (flags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) &&
          !(flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
        bool isHostVisible = (m_memTypes[i].memType.propertyFlags &
                              VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
        if (isHostVisible) {
          // Check if there is a better type (Device Local only) available later
          // in the list
          bool betterTypeExists = false;
          for (uint32_t j = i + 1; j < m_memProps.memoryTypeCount; j++) {
            if ((req->memoryTypeBits & (1u << j)) &&
                (m_memTypes[j].memType.propertyFlags & flags) == flags &&
                !(m_memTypes[j].memType.propertyFlags &
                  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
              betterTypeExists = true;
              break;
            }
          }
          if (betterTypeExists)
            continue; // Skip this type, wait for the better one
        }
      }

      result = this->tryAllocFromType(&m_memTypes[i], flags, req->size,
                                      req->alignment, hints, dedAllocInfo);
    }
  }
...
DxvkMemory DxvkMemoryAllocator::tryAllocFromType(
    DxvkMemoryType *type, VkMemoryPropertyFlags flags, VkDeviceSize size,
    VkDeviceSize align, DxvkMemoryFlags hints,
    const VkMemoryDedicatedAllocateInfo *dedAllocInfo) {
  ...
  if (size >= chunkSize || dedAllocInfo) {
    ...
    DxvkDeviceMemory devMem =
        this->tryAllocDeviceMemory(type, flags, size, hints, dedAllocInfo);

    if (devMem.memHandle != VK_NULL_HANDLE)
      memory = DxvkMemory(this, nullptr, type, devMem.memHandle, 0, size,
                          devMem.memPointer);
  } else {
    ...
    if (!memory) {
      ...
      for (uint32_t i = 0;
           i < 6 && (chunkSize >> i) >= size && !devMem.memHandle; i++)
        devMem =
            tryAllocDeviceMemory(type, flags, chunkSize >> i, hints, nullptr);

      if (devMem.memHandle) {
        Rc<DxvkMemoryChunk> chunk =
            new DxvkMemoryChunk(this, type, devMem, hints);
...
DxvkDeviceMemory DxvkMemoryAllocator::tryAllocDeviceMemory(
    DxvkMemoryType *type, VkMemoryPropertyFlags flags, VkDeviceSize size,
    DxvkMemoryFlags hints, const VkMemoryDedicatedAllocateInfo *dedAllocInfo) {
  bool useMemoryPriority =
      (flags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) &&
      (m_device->features().extMemoryPriority.memoryPriority);

  if (type->heap->budget &&
      type->heap->stats.memoryAllocated + size > type->heap->budget)
  ...
  if (m_vkd->vkAllocateMemory(m_vkd->device(), &info, nullptr,
                              &result.memHandle) != VK_SUCCESS)
    return DxvkDeviceMemory();

  if (flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
    VkResult status = m_vkd->vkMapMemory(m_vkd->device(), result.memHandle, 0,
                                         VK_WHOLE_SIZE, 0, &result.memPointer);
...
void DxvkMemoryAllocator::free(const DxvkMemory &memory) {
  std::lock_guard<dxvk::mutex> lock(m_mutex);
  memory.m_type->heap->stats.memoryUsed -= memory.m_length;

  if (memory.m_chunk != nullptr) {
    this->freeChunkMemory(memory.m_type, memory.m_chunk, memory.m_offset,
                          memory.m_length);
  } else {
    DxvkDeviceMemory devMem;
...
void DxvkMemoryAllocator::freeChunkMemory(DxvkMemoryType *type,
  ...
  if (chunk->isEmpty()) {
    ...
    type->chunks.erase(
        std::remove(type->chunks.begin(), type->chunks.end(), chunkRef));
...
void DxvkMemoryAllocator::freeDeviceMemory(DxvkMemoryType *type,
...
VkDeviceSize DxvkMemoryAllocator::pickChunkSize(uint32_t memTypeId,
...
bool DxvkMemoryAllocator::shouldFreeChunk(
    const DxvkMemoryType *type, const Rc<DxvkMemoryChunk> &chunk) const {
...
bool DxvkMemoryAllocator::shouldFreeEmptyChunks(
    const DxvkMemoryHeap *heap, VkDeviceSize allocationSize) const {
...
void DxvkMemoryAllocator::freeEmptyChunks(const DxvkMemoryHeap *heap) {
  for (uint32_t i = 0; i < m_memProps.memoryTypeCount; i++) {
    ...
    type->chunks.erase(std::remove_if(type->chunks.begin(), type->chunks.end(),
