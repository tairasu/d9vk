...
void D3D9MemoryAllocator::FreeChunk(D3D9MemoryChunk *Chunk) {
  ...
  m_chunks.erase(
      std::remove_if(m_chunks.begin(), m_chunks.end(),
                     [&](auto &item) { return item.get() == Chunk; }),
      m_chunks.end());
}
...
void D3D9MemoryAllocator::NotifyFreed(uint32_t Size) { m_usedMemory -= Size; }

uint32_t D3D9MemoryAllocator::MappedMemory() { return m_mappedMemory.load(); }

uint32_t D3D9MemoryAllocator::UsedMemory() { return m_usedMemory.load(); }
...
D3D9MemoryChunk::D3D9MemoryChunk(D3D9MemoryAllocator *Allocator, uint32_t Size)
    : m_allocator(Allocator), m_size(Size),
      m_mappingGranularity(m_allocator->MemoryGranularity() * 16) {
  ...
  m_mappingRanges.resize(
      ((Size + m_mappingGranularity - 1) / m_mappingGranularity));
}
...
void *D3D9MemoryChunk::Map(D3D9Memory *memory) {
  ...
  if (alignedSize > m_mappingGranularity) {
    // The allocation crosses the boundary of the internal mapping page it's a
    // part of so we map it on it's own.
    alignedOffset =
        alignDown(memory->GetOffset(), m_allocator->MemoryGranularity());
    ...
    uint8_t *basePtr = static_cast<uint8_t *>(MapViewOfFile(
        m_mapping, FILE_MAP_ALL_ACCESS, 0, alignedOffset, alignedSize));
    if (unlikely(basePtr == nullptr)) {
      DWORD error = GetLastError();
      Logger::err(
          str::format("Mapping non-persisted file failed: ", error,
                      ", Mapped memory: ", m_allocator->MappedMemory()));
      return nullptr;
    }
    return basePtr + alignmentDelta;
  }

  // For small allocations we map the entire mapping page to minimize the
  // overhead from having the align the offset to 65k bytes. This should
  // hopefully also reduce the amount of MapViewOfFile calls we do for tiny
  // allocations.
  auto &mappingRange =
      m_mappingRanges[memory->GetOffset() / m_mappingGranularity];
  if (unlikely(mappingRange.refCount == 0)) {
    m_allocator->NotifyMapped(m_mappingGranularity);
    mappingRange.ptr = static_cast<uint8_t *>(
        MapViewOfFile(m_mapping, FILE_MAP_ALL_ACCESS, 0, alignedOffset,
                      m_mappingGranularity));
    if (unlikely(mappingRange.ptr == nullptr)) {
      ...
      Logger::err(str::format("Mapping non-persisted file failed: ", error,
                              ", Mapped memory: ", m_allocator->MappedMemory(),
                              ", Msg: ", buffer));
      if (buffer) {
        LocalFree(buffer);
...
D3D9Memory D3D9MemoryChunk::Alloc(uint32_t Size) {
  ...
  for (auto range = m_freeRanges.begin(); range != m_freeRanges.end();
       range++) {
...
D3D9MemoryAllocator *D3D9MemoryChunk::Allocator() const { return m_allocator; }

HANDLE D3D9MemoryChunk::FileHandle() const { return m_mapping; }
...
D3D9Memory::~D3D9Memory() { this->Free(); }
...
void *D3D9Memory::Ptr() { return m_ptr; }
...
uint32_t D3D9MemoryAllocator::UsedMemory() { return m_allocatedMemory.load(); }
...
D3D9Memory::D3D9Memory(D3D9MemoryAllocator *pAllocator, size_t Size)
    : m_allocator(pAllocator), m_ptr(malloc(Size)), m_size(Size) {}
